...
fn rc_flow_unlock_manual_stabilize_position_rtl_land() {
    ...
    let (mt1, tr1, p1, _v1) = run_rc(&mut ctrl, &rc(0.5, 0.0, 0.0, true, 0), 1.0);
    ...
    assert_tru_bounded(&tr2, "rc stabilize", -5.0, 5.0, 45.0);

    // 切定点（槽3）：锚定保持点，先平移脱离原点，为返航制造实际行程。
    let (mt3, _tr3, _p3, _v3) = run_rc(&mut ctrl, &rc(0.5, 0.0, 0.0, true, 3), 1.0);
    ctrl.set_hold_pos(5.0, 0.0, -5.0); // 平移到北 5m
    let (mt3b, tr3b, p3b, _v3b) = run_rc(&mut ctrl, &rc(0.5, 0.0, 0.0, true, 3), 6.0);
    ...
    assert_tru_bounded(&tr3b, "rc position", -5.0, 8.0, 45.0);

    // 返航（槽4）：回到起飞点 home=(0,0,-5)，应有可测的北向回程。
    let (_mt4, tr4, p4, _v4) = run_rc(&mut ctrl, &rc(0.5, 0.0, 0.0, true, 4), 8.0);
    ...
    assert_tru_bounded(&tr4, "rc rtl", -5.0, 8.0, 45.0);

    // 降落（槽5）：目标降至地面（NED d=0），应显著下降（NED d 增大）。
    let d_rtl = p4[2];
    let (_mt5, tr5, p5, v5) = run_rc(&mut ctrl, &rc(0.3, 0.0, 0.0, true, 5), 3.0);
    ...
    assert_tru_bounded(&tr5, "rc land", -5.0, 8.0, 45.0);
}
...
fn rc_disarm_zeroes_thrust_and_descends() {
    let mut ctrl = new_ctrl();
    // 增稳悬停一段，稳定在 -5m。
    let (_m, tr0, p0, _v) = run_rc(&mut ctrl, &rc(0.5, 0.0, 0.0, true, 1), 2.0);
    assert!(ctrl.is_armed(), "遥控解锁后应 armed");
    assert_tru_bounded(&tr0, "rc hover before disarm", -5.0, 5.0, 45.0);

    // 遥控上锁（armed=false）：本拍起电机归零（free-fall），机体下落（NED d 增大）。
    let d_start = p0[2];
    let (_m, tr1, p1, v1) = run_rc(&mut ctrl, &rc(0.0, 0.0, 0.0, false, 1), 1.5);
    ...
    // 自由落体段真值仍有界：高度回落至地面、水平不 runaway、姿态不翻滚。
    assert_tru_bounded(&tr1, "rc free-fall after disarm", -5.0, 5.0, 45.0);
}
