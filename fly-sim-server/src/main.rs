...
// SYS 经 JOC_BASE_ELF、APP 经 JOC_APP_FLYCTRL_REAL（默认 /tmp/flyctrl_real.bin），
// 均由 mcu_simulater::artifact 解析（→ 壳工程布局 → 历史路径兜底）。
...
impl VperiphMc {
    ...
    fn boot_once() -> Result<Self, String> {
        ...
        m.load_app_partition(&mcu_simulater::artifact::flyctrl_real_app_bin())
