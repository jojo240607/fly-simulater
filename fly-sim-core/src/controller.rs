    ...
    W: RigidBodyWorld,
{
    ...
    pub fn new_at(
        ...
        // `att_alpha` 统一为 **0.02**（= 固件 `default_quad()` 的值）。
        //
        // ⚠️ 2026-09-21 决策：**H 场与固件必须同口径**，否则 H 场的闭环结论不可
        // 外推为 M 场验收依据。此前 SIL 用 0.0（"纯陀螺积分"），理由是"本侧观测密集、
        // 无需重力锚定"；但代价实测很实在：**H 场里 roll/pitch 没有任何重力修正**，
        // 陀螺零偏让姿态自由漂 ⇒ 无风档 60s 位置漂 9.36m 且仍在增长，量级远超固件侧
        // 应有水平（固件有 0.02 在压制）。而且陀螺零偏在线估计就挂在锚定块内，
        // att_alpha=0 会让它**根本不执行**（实测所有增益档位逐位相同）。
        //
        // 已知代价（改后需复核）：故障注入测例
        // `sensor_fault::gps_bias_step_offsets_estimate_bounded` 曾在 0.02 下翻滚
        // （tilt 81°，应 <45°）—— 原因是锚定把估计拉向比力反方向、而 GPS 偏置使
        // 估计跟随物理翻滚形成正反馈。该问题应**在固件侧一并解决**（固件本来就是
    ...
    pub fn step_rc(&mut self, rc: &RcInput) -> VehicleState {
        ...
        // ⚠️ **两场的"模式开关"是不同的硬件，语义本就不同**（2026-09-21 定案）：
        //
        // - **SIL（此处）**：模拟 **6 位档位开关**，`slot 0 = Manual`（无估计直通，
        //   是 SIL 独有的测试能力：可用于"绕过估计器"的对照实验）；
        // - **固件**：SBUS `ch[5]` 是 **3 位开关**（`sbus.rs`：<600→0 / <1400→1 /
        //   其余→2），最低档映射 `COPTER_MODE_STABILIZE`，**固件没有 Manual 档**。
        //
        // 曾一度用 `flightmode::mode_from_rc_switch` 把两边"统一"（按固件口径），
        // 代价是**打断了 SIL 的 `slot 0 = Manual` 约定**、`rc_modes` 两项失败。
        // **结论：这不是不一致，而是两个不同的开关模型。** "输入等价"要统一的是
        // **被测输入**（传感器/风场/硬铁），不是**模式枚举表** —— 强统会丢掉
        // "无估计直通"这一测试能力。
        //
        // 若将来确实要求两边模式语义一致，正确做法是在**固件侧**也支持 Manual 档
        // （而不是在 SIL 侧砍掉它）—— 属项目决策。
    ...
    /// HIL 模式：最近一次**磁力计**样本（机体系，与 `SensorConfig` 一致）。
    ///
    /// 用途（两场输入等价）：M 场把本值经 `st.mag` 覆盖通道喂给固件，使其磁输入
    /// 与 SIL 控制律看到的**完全同源**。
    /// 背景：M 场原先的磁力计由虚拟外设按 `st.att`（总是场景脚本真值=机体水平）
    /// 推导，而该测例用物理引擎推进机体 ⇒ 固件永远收到「机体水平」的假磁场，
    /// 与真实姿态对打（实测使 pitch 从 16.5° 恶化到 35.3°）。
