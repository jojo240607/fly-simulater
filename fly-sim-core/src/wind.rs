...
impl WindConfig {
    /// **验收风场：蒲福 3 级**（微风，3.4~5.4 m/s）之**上限**。
    ///
    /// # 需求（2026-09-21 定案）
    /// 飞行器**至少应能抗 3 级风**。本预设取**该级上限 5.4 m/s** 作基准风速 ——
    /// 用上限而非下限，避免"名义 3 级、实际只是下限"的验收放水。
    ///
    /// # ⚠️ 本函数是 H 场与 M 场的**唯一真源**
    /// 两场必须引用同一个预设。此前 H 场（`wind_turb_scan`）与 M 场（`x_hover_env`）
    /// **各写各的风场、参数还不一致** —— 那样"输入等价"无从谈起，
    /// H 场验证过的抗风结论也就无法作为 M 场验收依据。
    ///
    /// # 组成（逐项都在测试里有物理依据）
    /// - **恒定侧风 5.4 m/s**：北向为主，东向按 2.5:1.0 同比（斜向来风）；
    /// - **阵风** 1.2 m/s @ 0.12Hz（约 8s 周期）——真实风的常态；
    /// - **Dryden 湍流** σ=[0.3,0.1,-0.3] m/s、τ=0.5s；
    /// - **风切变** 幂律 α=0.2、参考 10m；**空间相关**尺度 2m；
    /// - **固定 seed** —— 保证可复现（两场必须逐位可比）。
    ///
    /// 峰值风速 ≈ 5.4 + 1.2 + 3σ ≈ 7.5 m/s，落在 **4 级下沿** ——
    /// 即"名义 3 级、阵风触 4 级"，这是真实天气的常态，验收应覆盖它。
    pub fn beaufort3() -> Self {
        Self {
            base: [5.4, 0.0, -2.16], // NED→UP: 北 5.4 + 东 2.16（2.5:1.0 同比）
            ...
            spatial_scale: 2.0,
            // 其余字段（阵风突发窗口、热气流等）沿用默认 —— 与本预设逐字对应旧
            // H/M 两场各自的配置（它们都是显式列同样几项 + Default 补齐）。
            ..Self::default()
        }
    }
}
