...
| 优先级 | 层 | 项目 | 状态 |
|---|---|---|---|
| **P3-A1** | 控制 | 轨迹跟踪控制器（倾斜补偿 + 速度/加速度前馈） | 待做（实证缺陷） |
| **P3-A2** | 控制 | `--sensor-noise` 下控制律鲁棒性（阶段 11 A/B/C 落地） | 待做 |
| **P3-A3** | 控制 | TECS 空速消费（总能量控制） | 待做 |
...
- [ ] **P3-A1 轨迹跟踪控制器**：倾斜补偿（`des_thrust /= cos(tilt)`）→ 速度/加速度前馈，目标 `mission` 长距离跟随 `stable=true`。
- [ ] **P3-A2 噪声鲁棒性**：定位是 IMU 姿态抖动还是 GPS 位置外环主导后，调参/加前馈或 EKF 输出低通，新增 `tests/sensor_noise.rs` 锁定 `TRU` 有界。
- [ ] **P3-A3 TECS**：空速进入控制律（与 EKF 空速融合衔接），风扰下能量保持。

#### P3-D：使用场景/生态层（Top 2/3）
- [ ] **P3-D1 RC 输入 + 全飞行模式**：遥控解锁 → 手动/增稳 → 自主任务 → RTL/降落 全流程场景（sim 侧注入 `RcInput`）。
